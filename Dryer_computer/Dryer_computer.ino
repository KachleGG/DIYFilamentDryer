...
void loop() {
  ...
  if (digitalRead(encoderButtonPin) == LOW) {
    ...
    if (settingTemperature) {
      ...
      while ((millis() - startTime) / 1000 < dryingTime) {
        sensors.requestTemperatures(); // Update temperature
        dryerTemperature = sensors.getTempCByIndex(0);
        ...
        lcd.setCursor(0, 1);
        lcd.print("Remain: ");
        lcd.print((dryingTime - (millis() - startTime) / 1000) / 60);
        lcd.print(" min");
